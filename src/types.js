...
const { $TYPE, $LAIRCLIENT, LairClientError } = require('./constants.js')
...
function assert_byte_size (bytes, expected_length, constructor) {
  log.debug(
    'Asserting that input (%s) has exactly %s bytes for type %s',
    () => [bytes.length, expected_length, constructor.name]
  )
  if (bytes.length > expected_length)
    throw new ConversionError(
      `Too many bytes: ${constructor.name} expected ${expected_length} bytes but there are ${bytes.length} bytes available`
    )
  else if (bytes.length < expected_length)
    throw new ConversionError(
      `Not enough bytes: ${constructor.name} expected ${expected_length} bytes but there are only ${bytes.length} bytes available`
    )
}
...
class LairType extends Uint8Array {
  ...
  static [Symbol.hasInstance] (instance) {
    if (instance[$TYPE] !== $LAIRCLIENT) return false

    if (instance.constructor.name === this.name) return true
    if (instance[Symbol.toStringTag] === this.name) return true
  ...
  static fromSource (src, strict = true) {
    if (this.SIZE === Infinity)
      throw new ConversionError(
        `Cannot use LairType.fromSource for types without a fixed size (${this.SIZE})`
      )
  ...
  static from (src, strict) {
    if (this.SIZE === Infinity)
      throw new ConversionError(
        `Cannot use LairType.from for types without a fixed size (${this.SIZE})`
      )
    ...
    if (!(src instanceof Uint8Array))
      throw new TypeError(
        `${
          this.name
        } can only be created from a Uint8Array: not '${typeof src}'`
      )
...
class LairString extends LairType {
  static fromSource (src, strict = true) {
    if (src.length < 8)
      throw new ConversionError(
        `Source (${src.length}) does not have enough bytes to get ${this.name} size (min. 8 bytes)`
      )
  ...
  static from (string) {
    if (typeof string !== 'string' && !(string instanceof String))
      throw new TypeError(
        ...
        } can only be created from type String: not '${typeof string}'`
      )
  ...
  static from (bytes) {
    if (!(bytes instanceof Uint8Array))
      throw new TypeError(
        `${
          this.constructor.name
        } can only be created from a Uint8Array: not '${typeof bytes}'`
      )
...
Object.defineProperty(LairCertPrivateKey, 'SIZE', {
  value: Infinity,
  writable: false
})
